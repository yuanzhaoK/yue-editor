...
const CustomNoteEditor = forwardRef<CustomNoteEditorRef, CustomNoteEditorProps>((props, ref) => {
  ...
  useEffect(() => {
    // 创建引擎实例
    const engine = new Engine("#custom-editor", {
      lang: "zh-cn",
    ...
    // 监听链接对话框事件
    engine.on('plugin:link:dialog:link', (data: any) => {
  ...
  // 当内容改变时更新（只在笔记切换时更新）
  useEffect(() => {
    if (isInitializedRef.current && engineRef.current && initialContent !== undefined) {
